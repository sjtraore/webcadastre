...
public class AddressHome {
	...
	public Address findById(int id) {
		...
		try {
			Address instance = (Address) sessionFactory.getCurrentSession().get("Address", id);
			if (instance == null) {
	...
	public List findByExample(Address instance) {
		...
		try {
			List results = sessionFactory.getCurrentSession().createCriteria("Address").add(Example.create(instance))
					.list();

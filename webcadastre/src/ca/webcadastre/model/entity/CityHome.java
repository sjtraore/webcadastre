...
public class CityHome {
	...
	public City merge(City detachedInstance) {
		...
		try {
			City result = (City) sessionFactory.getCurrentSession().merge(detachedInstance);
	...
	public City findById(java.lang.Integer id) {
		...
		try {
			City instance = (City) sessionFactory.getCurrentSession().get("City", id);
			if (instance == null) {
	...
	public List findByExample(City instance) {
		...
		try {
			List results = sessionFactory.getCurrentSession().createCriteria("City").add(Example.create(instance))
					.list();

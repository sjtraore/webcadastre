...
public class AreaHome {
	...
	public Area findById(AreaId id) {
		log.debug("getting Area instance with id: " + id);
		try {
			Area instance = (Area) sessionFactory.getCurrentSession().get("Area", id);
			if (instance == null) {
	...
	public List findByExample(Area instance) {
		...
		try {
			List results = sessionFactory.getCurrentSession().createCriteria("Area").add(Example.create(instance))
					.list();
